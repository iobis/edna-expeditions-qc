...
detections <- occurrence %>%
  ...
  group_by(higherGeography, decimalLongitude, decimalLatitude, species, class, family, group, aphiaid, vsearch) %>%
  summarize(batch = paste0("b", str_replace_all(paste0(unique(sort(eventRemarks)), collapse = ","), "sequencing batch ", ""))) %>%
  ungroup() %>%
...
detections <- detections %>%
  ...
  select(site = higherGeography, batch, class, family, species, vsearch, group, distance, worms, thermal, score, aphiaid) %>%
